...
import { filterProjectsByCategory, PROJECT_FILTERS } from '../../utils/projectFilters';

export default function ProjectsSection({ projects, onOpenDemo }) {
  const [selectedFilter, setSelectedFilter] = useState("All");
  const filteredProjects = useMemo(
    () => filterProjectsByCategory(projects, selectedFilter),
    ...
    <div className="p-6 rounded-2xl border border-neutral-200 bg-white flex flex-col h-full relative transition-all duration-300 shadow-sm hover:border-neutral-400/60">
      ...
      <div className="flex flex-wrap gap-1.5 mb-6">
        {project.tech.map((tech) => (
          <span key={tech} className="text-[10px] font-bold text-neutral-700 bg-neutral-50 border border-neutral-200 px-2 py-0.5 rounded">
            {tech}
          </span>
        ))}
      </div>
